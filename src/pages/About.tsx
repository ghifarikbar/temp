...
const PilarCard = ({
  ...
  return (
    <div
      id="anak"
      className="relative h-80 w-80 overflow-hidden rounded-bl-[50px] rounded-tr-[50px] lg:w-60">
      <div className="absolute left-0 top-4 h-full w-full px-4">
        <h1 className="text-5xl font-bold text-[#F4F4F4] lg:text-3xl">
          {title}
        </h1>
...
const CardAbout = () => {
  return (
    <div className="mt-20 flex w-full flex-col items-center  justify-center gap-0 overflow-hidden bg-[#2E323A] lg:flex-row lg:gap-16">
      ...
      <div
        id="text"
        className="h-full w-full justify-end self-center px-6 py-12 text-white sm:px-12
           lg:w-3/6">
        {/* <h1 className="text-5xl font-bold text-[#F4F4F4]">Tentang Kami</h1> */}
        <p className="text-lg text-[#F4F4F4] md:text-4xl lg:text-5xl">
          “Rambut lebih dari sekedar gaya. Ini merupakan bentuk ekspresi diri,
          cara untuk menampilkan kepribadian unik kita kepada dunia, dan simbol
          kegembiraan dan kreativitas kami.”
        </p>
      </div>
    </div>
  );
      ...
      <div id="image" className="w-full lg:w-3/6">
        <figure
          className={cn(
            ...
            isOdd ? "pt-0 lg:pt-20" : "pb-0 lg:pb-20"
          )}>
